...
export default function VocabularyPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!word.trim() || !contextSentence.trim() || !translatedDefinition.trim()) return;
    ...
    try {
      const res = await fetch('/api/vocabulary', {
        ...
        body: JSON.stringify({ word, contextSentence, translatedDefinition }),
      });
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)' }} className="text-sm sm:text-base">Build and review your personal word bank.</p>
        </div>
        ...
        <Card className="p-5 sm:p-6">
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            ...
            <Input
              ...
              required
            />
            <Button type="submit" isLoading={saving} disabled={!word.trim() || !contextSentence.trim() || !translatedDefinition.trim()}>
              Save Word
            </Button>
          </form>
        </Card>
        ...
        <Card className="p-8 sm:p-12 text-center">
          <div className="space-y-4">
            ...
            <p style={{ color: 'var(--text-secondary)' }} className="max-w-md mx-auto text-sm sm:text-base">
              Start building your word bank by adding new words you encounter during your studies.
            </p>
            ...
            <Card key={entry._id} className="p-4 sm:p-5 flex flex-col group relative">
              <div className="flex items-start justify-between gap-2">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-1.5 rounded-lg text-sm"
      ...
      {entries.length > 0 && (
        <div className="text-center text-sm pb-4" style={{ color: 'var(--text-muted)' }}>
          {entries.length} word{entries.length !== 1 ? 's' : ''} in your vocabulary
        </div>
      )}
    </div>
  );
